...
based on tutorial: http://pytorch.org/tutorials/intermediate/seq2seq_translation_tutorial.html
"""
...
MAX_LENGTH = 35  # query max length
...
class Lang(object):
    def __init__(self, name, vocab_path):
        ...
        self.word2index, self.index2word = self.initialize_vocab(vocab_path)
        self.n_words = len(self.index2word)

    def initialize_vocab(self, vocab_path):
...
input_lang = Lang("src", pjoin("data", dataset, "src_vocab.dat"))
output_lang = Lang("tgt", pjoin("data", dataset, "tgt_vocab.dat"))
...
def train(input_variable, target_variable, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion,
    ...
    # need to clean out previous tapes
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    ...
    decoder_input = Variable(torch.LongTensor([[SOS_ID]]))  # SOS
    decoder_input = decoder_input.cuda() if use_cuda else decoder_input
...
q_train = filterPairs(q_train)
q_valid = filterPairs(q_valid)
...
trainIters(encoder1, attn_decoder1, q_train, 75000, print_every=5000)

evaluateRandomly(encoder1, attn_decoder1, q_valid)