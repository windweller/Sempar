...
# (env, logic) -> (env')
# so this also needs null condition
...
class NLCModel(object):
    ...
    def setup_embeddings(self):
        with vs.variable_scope("embeddings"):
            ...
            self.encoder_inputs = embedding_ops.embedding_lookup(self.L_dec, self.target_tokens)
    ...
    def concate_encode(self):
        ...
        query_w_matrix = self.normal_encode(self.encoder_inputs, self.target_mask,
    ...
    def train_engine(self, session, target_tokens, target_mask, ctx_tokens, ctx_mask, pred_tokens, pred_mask):
        input_feed = {}
    ...
    def test_engine(self, session, target_tokens, target_mask, ctx_tokens, ctx_mask, pred_tokens, pred_mask):
        input_feed = {}
        input_feed[self.target_tokens] = target_tokens
        input_feed[self.ctx_tokens] = ctx_tokens
        input_feed[self.pred_tokens] = pred_tokens
    ...
    def encode_engine(self, session, target_tokens, target_mask, ctx_tokens, ctx_mask):
        input_feed = {}
        input_feed[self.target_tokens] = target_tokens
        input_feed[self.ctx_tokens] = ctx_tokens
        input_feed[self.target_mask] = target_mask
