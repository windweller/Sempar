...
tf.app.flags.DEFINE_integer("size", 250, "Size of each model layer.")
...
def create_model(session, src_vocab_size, tgt_vocab_size, forward_only):
    model = rnn_nlc.NLCModel(
...
def decode_validate(model, sess, q_valid, reverse_src_vocab, reverse_tgt_vocab, save_dir,
    ...
    with open(pjoin(save_dir, "valid_decode_e" + str(epoch) + ".txt"), "wb") as f:
        for source_tokens, source_mask, target_tokens, target_mask in pair_iter(q_valid, 1,
                                                                                FLAGS.input_len, FLAGS.query_len):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        if not FLAGS.dev:
            ...
            while (FLAGS.epochs == 0 or epoch < FLAGS.epochs):
                ...
                f1, em, saved_list = decode_validate(model, sess, q_valid, rev_src_vocab, rev_tgt_vocab, decode_save_dir, epoch, sample=5)
            ...
            # Validate by decoding
            f1, em, saved_list = decode_validate(model, sess, q_valid, rev_src_vocab, rev_tgt_vocab,
